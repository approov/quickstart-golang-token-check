...
func TestUnmarshal_StdTestSuite(t *testing.T) {
	testCases, err := parseTestFiles("structured_field_tests/*.json")
	if err != nil {
...
func TestMarshal_StdSerializationTestSuite(t *testing.T) {
	testCases, err := parseTestFiles("structured_field_tests/serialisation-tests/*.json")
	if err != nil {
		t.Errorf("parse test files: %v", err)
		return
	}
...
func decodeBareItem(v interface{}) sfv.BareItem {
	...
	case map[string]interface{}:
		...
		case "date":
			// In the httpwg tests this is seconds since epoch as a JSON number.
			num, ok := v["value"].(json.Number)
			if !ok {
				panic("date value not a json.Number")
			}
			secs, err := num.Int64()
			if err != nil {
				panic(err)
			...
			return sfv.BareItem{
				Type: sfv.BareItemTypeDate,
				Date: time.Unix(secs, 0).UTC(),
			}
		case "displaystring":
			return sfv.BareItem{
				Type:          sfv.BareItemTypeDisplayString,
				DisplayString: v["value"].(string),
			}
		default:
			panic("bad __type")
		}
